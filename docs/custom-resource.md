...
| `with.volumeSize`                      | object               |                                                   | Size of persistent volume claims for components.                                                                                             |
| `with.volumeSize.mongoDbData`          | quantity             | 8Gi                                               | Size of the data directory for the MongoDb server.                                                                                           |
| `with.volumeSize.mysqlData`            | quantity             | 8Gi                                               | Size of the data directory for the MySQL server.                                                                                             |
| `with.volumeSize.solrData`             | quantity             | 8Gi                                               | Size of the data directory for the Solr server. Each Solr instance will have its own PVC.                                                    |
| `with.volumeSize.transformedBlobCache` | quantity             | 8Gi                                               | Size of the transformed BLOB cache. Each CoreMedia component has their own PVC; RLS and CAE will have one each.                              |
| `with.volumeSize.uapiBlobCache`        | quantity             | 8Gi                                               | Size of the UAPI BLOB cache. Each CoreMedia component has their own PVC; RLS and CAE will have one each.                                     |
...
Some components require persistent storage, for example most UAPI clients, the database servers, or Solr. Using `with.volumeSize`, you can customize the amount of space the PVCs and PVs are created with. **Note** the operator does not know how to resize a volume. If you need to change the size of a volume after it has been created, you will need to employ standard Kubernetes mechanisms to do so.
