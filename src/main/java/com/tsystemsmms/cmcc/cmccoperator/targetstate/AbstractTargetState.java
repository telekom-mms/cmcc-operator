...
public abstract class AbstractTargetState implements TargetState {
    ...
    final Map<String, Map<String, ClientSecretRef>> clientSecretRefs = new HashMap<>();
    final Map<String, Map<String, DefaultClientSecret>> clientSecrets = new HashMap<>();
    ...
    public void buildClientSecretRefs() {
        ...
        for (Map.Entry<String, Map<String, ClientSecretRef>> perKind : getCmcc().getSpec().getClientSecretRefs().entrySet()) {
            Map<String, ClientSecretRef> managed = clientSecretRefs.computeIfAbsent(perKind.getKey(), k -> new HashMap<>());
            managed.putAll(perKind.getValue());
        }
...
//            throw new IllegalArgumentException("Component needs a client secret for " + kind + "/" + schema + ", but none exists");
            if (kindRefs == null) {
                kindRefs = new HashMap<>();
                clientSecretRefs.put(kind, kindRefs);
            }
            Map<String, DefaultClientSecret> perKind = clientSecrets.computeIfAbsent(kind, k -> new HashMap<>());
            DefaultClientSecret defaultClientSecret = perKind.computeIfAbsent(schema, k -> buildDefaultClientSecret.apply(getClientPassword()));
            kindRefs.put(schema, defaultClientSecret.getRef());
        }
    ...
    public boolean isComponentStatefulSetReady(ComponentSpec cs) {
        ...
        return isStatefulSetReady(getStatefulSetFor(component), getServiceNameFor(component));
    }

    public boolean isStatefulSetReady(List<StatefulSet> statefulSets, String label) {
        ...
        StatefulSetStatus status = statefulSets.get(0).getStatus();
        log.debug("sts {}: replicas {} / available {}", statefulSets.get(0).getMetadata().getName(), status.getReplicas(), status.getReadyReplicas());
