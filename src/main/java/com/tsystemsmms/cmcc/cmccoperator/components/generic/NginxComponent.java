...
/**
 * Run nginx to serve static files. This runs the standard NGINX image (docker.io/library/nginx), but expects nginx to
 * listen on port 8080 (so we can run it as non-root). With the default image, this is accomplished by setting
 * NGINX_PORT=8080.
 */
@Slf4j
public class NginxComponent extends AbstractComponent implements HasService {

    public NginxComponent(KubernetesClient kubernetesClient, TargetState targetState, ComponentSpec componentSpec) {
        super(kubernetesClient, targetState, componentSpec, "nginx");
    }
